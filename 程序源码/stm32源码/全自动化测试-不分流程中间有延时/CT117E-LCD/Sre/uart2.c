...
u8 USART2_TX_BUF[USART2_SEND_LEN]; 			//发送缓冲,最大USART3_MAX_SEND_LEN字节
//接收状态
//bit15，	接收完成标志
//bit14，	接收到0x0d
//bit13~0，	接收到的有效字节数目
...
/**
  ...
  * 说    明：无
  */
void USART2_SendBytes(u8* data,int32_t len)
{ 
	while(len)
	{
...
{  
	...
	/*依次处理含有未知数目和类型的函数变元表*/
	va_list ap;        //类型为 va_list 的变量 ap，它将依次指向每个变元  访问任何未命名的变元前必须用 va_start 宏对 ap 进行初始化
	va_start(ap,fmt); 
